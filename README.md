# 通信服务器和客户端实现。
> 此项目服务器基本框架为 网络层、 服务层、 数据存储层 
> 网络层采用的是开源的muduo网络库，是基于非阻塞IO和事件驱动的C++高并发TCP网络库，该网络库很好的解耦了网络模块和业务模块的代码。
> 服务层这里用了一些C++11的技术，例如绑定器、
> 数据存储层，使用MYSQL数据库对项目中的一些重要的数据进行落地存储，比如用户账号密码，好友列表，离线消息。
